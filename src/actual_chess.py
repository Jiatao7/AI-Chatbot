...
class ChessGame:
    ...
    def ai_make_move(self):
        global board
        try:
            prompt = f"The current board position in FEN notation is: {board.fen()}. You are playing as {self.ai_side}. Provide the best move in standard chess notation where the notation is only the piece and where the piece moves to (example, pawn to e2 = e2, knight to f3 = Nf3, queen to a5 = Qa5). Be decent at the game, but not an completely optimized player. Play like a 1600 elo player. Provide 50 moves seperated by spaces, in order of how good you think the move is."

            response = aiplayer.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are a chess bot. Respond with a move in standard chess notation. Be decent at the game, but not an completely optimized player. Play like a 1600 elo player. Provide 50 moves seperated by spaces, in order of how good you think the move is."},
                    {"role": "user", "content": prompt}
                ]
            )
            # print(response)
        ...
        # this would work if chatgpt was ACTUALLY GOOD AT CHESS AND HAD A TODDLER'S IQ FOR OBJECT PERMANENCE
